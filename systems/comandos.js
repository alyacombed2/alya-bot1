...
module.exports = (client) => {
  ...
  const ALLOWED_COMMANDS = [
    ...
    "apostar","assaltar","loja","comprar","inventario","inv","usar",
    "perfil","rankmoney","ranklevel","rankmsg","ppt","caraoucoroa","dado","adivinhe",
  ...
  function getUser(id) {
    if (!users[id]) {
      users[id] = {
        ...
        messages: 0,inventory: [],marriedTo: null,daily: 0,work: 0
      };
    }
    return users[id];
  }
  ...
  client.on("messageCreate", async (message) => {
    ...
    if (cmd === "ship") {
      ...
      if (!u1 || !u2 || u1.id === u2.id) return message.reply("❌ Use: `!ship @pessoa1 @pessoa2`");
    ...
    if (cmd === "8ball") {
      ...
      const respostas = ["Sim.","Não.","Talvez.","Com certeza.","Nunca.","Provavelmente.","Muito suspeito...","Isso vai dar ruim.","Confia."];
    ...
    if (cmd === "saldo" || cmd === "money") return message.reply(`💰 ${message.author}, você tem **${user.money} moedas**.`);
    if (cmd === "daily") {
      const now = Date.now();
    ...
    if (cmd === "work" || cmd === "trabalhar") {
      ...
      const jobs = ["vendeu água no semáforo","programou um bot bugado","lavou uma capivara","editou vídeo de Free Fire","foi CLT por 1 hora","farmou no Minecraft"];
    ...
    if (cmd === "loja") {
      ...
      return message.channel.send({ embeds: [createEmbed("🛒 Loja do servidor", itens, "Blue")] });
    }
    if (cmd === "comprar") {
      ...
      if (user.money < item.price) return message.reply("❌ Você não tem dinheiro suficiente.");
      user.money -= item.price;
      user.inventory.push(nome);
      return message.reply(`🛍️ Você comprou **${nome}** por **${item.price} moedas**.`);
    }
    if (cmd === "inventario" || cmd === "inv") {
      if (user.inventory.length === 0) return message.reply("🎒 Seu inventário está vazio.");
      return message.channel.send({ embeds: [createEmbed(`🎒 Inventário de ${message.author.username}`, user.inventory.map(i => `• ${i}`).join("\n"), "Purple")] });
    }
    if (cmd === "usar") {
      ...
      if (item === "uno reverso") return message.reply("🔄 Você usou o **Uno Reverso**. Absolutamente nada aconteceu... mas foi estiloso.");
      if (item === "capivara") return message.reply("🐹 Sua capivara te observou em silêncio. Você sente respeito.");
      if (item === "miojo sagrado") {
        user.money += 300;
        return message.reply("🍜 Você comeu o **Miojo Sagrado** e achou **300 moedas** dentro.");
      }
      if (item === "chinelo divino") return message.reply("🩴 Você invocou o **Chinelo Divino**. O medo tomou conta do servidor.");
    ...
    if (cmd === "rankmoney") {
      const top = Object.entries(users).sort((a, b) => b[1].money - a[1].money).slice(0, 10);
      let desc = "";
      for (let i = 0; i < top.length; i++) {
        const userObj = await client.users.fetch(top[i][0]).catch(() => null);
        desc += `**${i + 1}.** ${userObj ? userObj.username : "Usuário"} — 💰 ${top[i][1].money}\n`;
      }
      return message.channel.send({ embeds: [createEmbed("💸 Ranking de dinheiro", desc || "Ninguém ainda.", "Gold")] });
    }
    if (cmd === "ranklevel") {
      const top = Object.entries(users).sort((a, b) => b[1].level - a[1].level || b[1].xp - a[1].xp).slice(0, 10);
      let desc = "";
      for (let i = 0; i < top.length; i++) {
        const userObj = await client.users.fetch(top[i][0]).catch(() => null);
        desc += `**${i + 1}.** ${userObj ? userObj.username : "Usuário"} — ⭐ Nível ${top[i][1].level}\n`;
      }
      return message.channel.send({ embeds: [createEmbed("🏆 Ranking de nível", desc || "Ninguém ainda.", "Green")] });
    }
    if (cmd === "rankmsg") {
      const top = Object.entries(users).sort((a, b) => b[1].messages - a[1].messages).slice(0, 10);
      let desc = "";
      for (let i = 0; i < top.length; i++) {
        const userObj = await client.users.fetch(top[i][0]).catch(() => null);
        desc += `**${i + 1}.** ${userObj ? userObj.username : "Usuário"} — 💬 ${top[i][1].messages} msgs\n`;
      }
      return message.channel.send({ embeds: [createEmbed("💬 Ranking de mensagens", desc || "Ninguém ainda.", "Orange")] });
    }
    if (cmd === "ppt") {
      const escolha = args[0]?.toLowerCase();
      const opcoes = ["pedra", "papel", "tesoura"];
      if (!opcoes.includes(escolha)) return message.reply("❌ Use: `!ppt pedra/papel/tesoura`");
    ...
    if (cmd === "caraoucoroa") {
      const escolha = args[0]?.toLowerCase();
      if (!["cara", "coroa"].includes(escolha)) return message.reply("❌ Use: `!caraoucoroa cara/coroa`");
    ...
    if (cmd === "adivinhe") {
      ...
      if (!palpite || palpite < 1 || palpite > 10) return message.reply("❌ Use: `!adivinhe 1-10`");
      if (palpite === num) {
        user.money += 500;
    ...
    if (cmd === "fakeban") {
      const alvo = message.mentions.users.first();
      if (!alvo) return message.reply("❌ Marque alguém.");
      return message.channel.send({ embeds: [createEmbed("🔨 Usuário banido", `**${alvo.tag}** foi banido do servidor.\n\nMotivo: **ser muito ruim**`, "Red")] });
    }
    if (cmd === "fakemute") {
      const alvo = message.mentions.users.first();
      if (!alvo) return message.reply("❌ Marque alguém.");
      return message.channel.send({ embeds: [createEmbed("🔇 Usuário silenciado", `**${alvo.tag}** foi mutado por **falar merda demais**.`, "DarkGrey")] });
    }
    if (cmd === "fakekick") {
      const alvo = message.mentions.users.first();
      if (!alvo) return message.reply("❌ Marque alguém.");
      return message.channel.send({ embeds: [createEmbed("👢 Usuário expulso", `**${alvo.tag}** foi expulso por ser **um perigo à sociedade**.`, "Orange")] });
    }
    ...
    if (cmd === "ajuda" || cmd === "help") {
      return message.channel.send({
        embeds: [createEmbed("📖 Comandos do Bot", `
**🎉 Diversão**
